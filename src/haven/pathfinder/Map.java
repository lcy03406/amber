...
public class Map {
    ...
    public final static int plbbox = 3;
    private final static int way = plbbox + 1;
    private final static int clr = plbbox + 2;
    ...
    // Dummy container for bounding box coordinates.
    // A bit odd implementation but at least it doesn't lead to unnecessary object instantiation each time.
    ...
    private static final BBox _bbox = new BBox();
    ...
    private BBox getGobBoundingBox(Gob gob) {
        Resource res = null;
        try {
            res = gob.getres();
        } catch (Loading l) {
    ...
    public void addGob(Gob gob) {
        BBox bbox = getGobBoundingBox(gob);
        if (bbox == null)
            return;
        ...
        // calculate waypoints located on the angular bisector of the corner
        Coord wa = Utils.rotate(gcx + bboxa.x - way, gcy + bboxa.y - way, gcx, gcy, cos, sin);
        Coord wb = Utils.rotate(gcx + bboxb.x + way, gcy + bboxa.y - way, gcx, gcy, cos, sin);
        Coord wc = Utils.rotate(gcx + bboxb.x + way, gcy + bboxb.y + way, gcx, gcy, cos, sin);
        Coord wd = Utils.rotate(gcx + bboxa.x - way, gcy + bboxb.y + way, gcx, gcy, cos, sin);

        // calculate TO clearance vertices
        Coord clra = Utils.rotate(gcx + bboxa.x - clr, gcy + bboxa.y - clr, gcx, gcy, cos, sin);
        Coord clrb = Utils.rotate(gcx + bboxb.x + clr, gcy + bboxa.y - clr, gcx, gcy, cos, sin);
        Coord clrc = Utils.rotate(gcx + bboxb.x + clr, gcy + bboxb.y + clr, gcx, gcy, cos, sin);
        Coord clrd = Utils.rotate(gcx + bboxa.x - clr, gcy + bboxb.y + clr, gcx, gcy, cos, sin);
        ...
        // plot bounding box
        HashMap<Integer, Utils.MinMax> raster = Utils.plotRect(map, ca, cb, cc, cd);
    ...
    private void sanitizeWaypoints() {
        ...
        // cluster vertices
        // this is done by semgneting the grid into 2* x 2* subgrid
        /// geometric center
        */
        }
    }
