...
public class Utils {
    ...
    public static Coord rotate(int x, int y, int pivotx, int pivoty, double cos, double sin) {
        x -= pivotx;
        y -= pivoty;
        return new Coord((int) Math.round(x * cos - y * sin) + pivotx, (int) Math.round(x * sin + y * cos) + pivoty);
    }
    ...
    public static HashMap<Integer, MinMax> plotRect(byte[][] map, Coord ca, Coord cb, Coord cc, Coord cd) {
        ...
        Utils.plotLine(map, raster, ca.x, ca.y, cb.x, cb.y, Map.CELL_BLK);
        Utils.plotLine(map, raster, cb.x, cb.y, cc.x, cc.y, Map.CELL_BLK);
        Utils.plotLine(map, raster, cd.x, cd.y, cc.x, cc.y, Map.CELL_BLK);
        Utils.plotLine(map, raster, ca.x, ca.y, cd.x, cd.y, Map.CELL_BLK);
